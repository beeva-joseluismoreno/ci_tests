...
public class RepCRMRegexpHBaseFeedbackEventSerializer implements HbaseEventSerializer {
	...
	public static final String defaultDate = "01/12/2015";
	/*
	...
	private static final Logger logger = LoggerFactory.getLogger(RepCRMRegexpHBaseFeedbackEventSerializer.class);
...
//							(random + "_" + values[0] + values[7].split(" ")[0] + values[3]).getBytes(Charsets.UTF_8);
					...
					//rowKey = (values[0] + values[7].split(" ")[0] + values[3]).getBytes(Charsets.UTF_8);
					rowKey = (random + "_" + values[0] + defaultDate + values[3]).getBytes(Charsets.UTF_8);
...
//				      rr);
			...
			filterByCamp.setFilterIfMissing(true);
			filterByToken.setFilterIfMissing(true);
			filterByFLanz.setFilterIfMissing(true);
			filterBySend.setFilterIfMissing(true);
...
//			logger.info(":::::::::::::::::::::::::::::::::::RepCRMRegexpHBaseFeedbackEventSerializer :: 10-4444");
	...
	public static void main(String[] args) {
		...
		RepCRMRegexpHBaseFeedbackEventSerializer me = new RepCRMRegexpHBaseFeedbackEventSerializer();

		Configuration config = HBaseConfiguration.create();
		Connection connection = null;
		Table table = null;

		me.tableName = "KBDP:TKBDPFDB";
		me.cf = "fdb".getBytes();

		try {
			connection = ConnectionFactory.createConnection(config);
			table = connection.getTable(TableName.valueOf("KBDP:TKBDPFDB"));
